        ...
        isEncrypted
            ? storageToken.value
                ? // None of the token matches
                    ...
                    encryptData.value.admin.tokens.every((hash) => !compareSync(storageToken.value, hash))
                : // None of the token matches
                    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                    encryptData.value.admin.tokens.every((hash) => !compareSync(sessionToken.value, hash))
            : false;
        return {
            isEncrypted,
            isLocked,
